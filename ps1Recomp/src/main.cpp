...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // Emit a sorted static dispatch table. The previous std::unordered_map
    // instantiated a large template in the already-huge generated translation
    // unit, consumed significant compiler memory, allocated at startup, and
    // required a runtime population pass. A flat table + binary search is
    // deterministic, allocation-free, and only needs ~10 comparisons for the
    // current ~1k-function KERNEL.
    result_cpp += "\n// Dispatch Table\n";
    result_cpp += "// Sorted PS1 address -> recompiled function pointer table\n\n";
    result_cpp += "#include <cstdlib>\n"; // getenv, abort
    result_cpp += "#include <cstring>\n"; // strcmp
    result_cpp += "#include <cstdio>\n\n"; // fflush
    result_cpp +=
        "typedef void (*recomp_func_t)(uint8_t*, recomp_context*);\n"
        "struct recomp_entry_t { uint32_t addr; recomp_func_t fn; };\n\n";

    result_cpp += "static const recomp_entry_t recomp_func_table[] = {\n";
    for (const auto &[addr, name] : addr_to_name) {
      result_cpp += fmt::format("    {{0x{:08X}u, {}}},\n", addr, name);
    }
    result_cpp += "};\n";
    result_cpp +=
        "static constexpr std::size_t recomp_func_count = "
        "sizeof(recomp_func_table) / sizeof(recomp_func_table[0]);\n";
    result_cpp += "static bool recomp_table_ready = false;\n\n";

    // Kept as a public hook for the harness; static table needs no population.
    result_cpp += "void recomp_init_dispatch_table() { recomp_table_ready = true; }\n\n";

    result_cpp += "recomp_func_t recomp_lookup(uint32_t addr) {\n";
    result_cpp += "    std::size_t lo = 0, hi = recomp_func_count;\n";
    result_cpp += "    while (lo < hi) {\n";
    result_cpp += "        const std::size_t mid = lo + ((hi - lo) >> 1);\n";
    result_cpp += "        const uint32_t key = recomp_func_table[mid].addr;\n";
    result_cpp += "        if (addr < key) hi = mid;\n";
    result_cpp += "        else if (addr > key) lo = mid + 1;\n";
    result_cpp += "        else return recomp_func_table[mid].fn;\n";
    result_cpp += "    }\n";
    result_cpp += "    return nullptr;\n";
    result_cpp += "}\n\n";
